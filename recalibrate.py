...
import torch
from modules import PointRecalibrationModel, AverageRecalibrationModel, DistributionRecalibrationModel, GaussianNLLModel, GaussianLaplaceMixtureNLLModel, NoRecalibrationModel, IterativePointRecalibrationModel
from pytorch_lightning.loggers import TensorBoardLogger
import numpy as np
from data_loaders import get_credit_regression_dataloader, get_uci_dataloaders, get_satellite_dataloaders, get_recalibration_dataloaders
...
def get_dataset(dataset, seed, train_frac, combine_val_train):
    ...
    if dataset in ["satellite", "combined_satellite", "uganda", "tanzania", "rwanda", "malawi", "mozambique", "zimbabwe"]:
        train, val, test, in_size, output_size, y_scale = get_satellite_dataloaders(name=dataset, split_seed=seed, batch_size=batch_size, combine_val_train=combine_val_train)
    else:
...
#            callbacks = [early_stop_callback],
    ...
    elif "Distribution" in model.__class__.__name__ or "Average" in model.__class__.__name__ or "IterativePoint" in model.__class__.__name__:
        model.training_step()
        if val:
...
def main(dataset="protein", seed=0, save="real", loss="point_calibration_loss", posthoc_recalibration=None, train_frac=1.0, num_layers=2, n_dim=100, epochs=500, n_bins=None, flow_type=None, learning_rate=1e-3, combine_val_train = False, val_only=False):
    ...
    elif "distribution" == posthoc_recalibration or "iterative_point" == posthoc_recalibration:
    ...
    if posthoc_recalibration == "point":
        ...
        recalibration_model = train_recalibration_model(recalibration_model, epochs, logname=logname, actual_datasets=dataloaders)
        ...
        if n_bins == None:
            n_bins = int(math.sqrt(dist_datasets[1].shape[0]))
            recalibration_parameters["n_bins"] = n_bins
        recalibration_model = IterativePointRecalibrationModel(dist_datasets, y_scale=y_scale, n_bins=n_bins, num_layers=num_layers)
        recalibration_model = train_recalibration_model(recalibration_model, 1, logname=None, actual_datasets=(train, val, test))
