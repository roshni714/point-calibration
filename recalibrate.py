...
def main(
    ...
    train, val, test, in_size, y_scale = get_dataset(
        dataset, seed, train_frac, combine_val_train, resnet
    )
    ...
    model = model_class.load_from_checkpoint(
        model_path, input_size=in_size[0], y_scale=y_scale, resnet=resnet
    )
    if resnet:
        print(resnet)
    print("loaded model")
    ...
    if posthoc_recalibration == "point":
        if val_only:
            logname = (
                "{}_val_{}_sigmoid_{}layers_{}_{}dim_{}bins_{}epochs_{}lr_{}".format(
                    dataset,
                    loss,
                    num_layers,
                    flow_type,
        ...
        if n_bins == None and loss == "gaussian_nll":
            ...
            recalibration_parameters["n_bins"] = n_bins
        del model
        recalibration_model = DistributionRecalibrationModel(
            dist_datasets, y_scale=y_scale, n_bins=n_bins
        )
        recalibration_model = train_recalibration_model(
            recalibration_model, 1, logname=None, actual_datasets=(train, val, test)
        )
