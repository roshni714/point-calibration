...
class SigmoidFlow2D(nn.Module):
        def __init__(self, num_layers, n_dim):
            ...
            l.append(SigmoidLayer(in_dim=n_dim, out_dim=1))
            self.layers =  nn.ModuleList(l)
            self.mlp = torch.nn.Sequential(torch.nn.Linear(1, 50), torch.nn.ReLU(),torch.nn.Linear(50, 50), torch.nn.ReLU(), torch.nn.Linear(50, 1))
        ...
        def cdf(self, x, fy0):
            ...
            if len(in_shape) == 2:
                ...
                final = res.reshape(in_shape)
                bottom = self.forward(torch.zeros(x.shape).to(device).flatten(), fy0.flatten()).reshape(in_shape)
                top = self.forward(torch.ones(x.shape).to(device).flatten(), fy0.flatten()).reshape(in_shape)
            else:
