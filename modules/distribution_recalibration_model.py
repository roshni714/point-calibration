...
#        n = max(1, math.ceil(math.log(self.n_bins)/math.log(n_params)))
        ...
        for i in range(len(self.train_dist.params)):
            param_bins = torch.linspace(torch.min(self.train_dist.params[i]), torch.max(self.train_dist.params[i]), n)
        ...
        cartesian_product = itertools.product(range(n-1), repeat=len(self.train_dist.params))
        for tup in tqdm(cartesian_product):
            condition = torch.ones(self.train_dist.params[i].shape).bool()
            for i in range(len(self.train_dist.params)):
                condition = torch.logical_and(condition, (self.train_dist.params[i] < parameter_bins[i][tup[i]+1]) & (self.train_dist.params[i] > parameter_bins[i][tup[i]]))
            ...
            Y = np.array([(k+1)/(len(sorted_forecasts) +2) for k in range(len(sorted_forecasts))])
