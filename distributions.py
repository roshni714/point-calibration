...
class GaussianLaplaceMixtureDistribution:
    ...
    def mean(self):
        return (
            self.weight * self.gaussian_comp.mean
    ...
    def std(self):
        ...
        laplace_part = (1 - self.weight) * (
            torch.pow(self.laplace_comp.mean, 2) + self.laplace_comp.variance
        )
    ...
    def to(self, device):
        ...
        self.laplace_comp = D.Laplace(
            self.laplace_comp.mean.to(device), self.laplace_comp.scale.to(device)
        )
    ...
    def detach(self):
        ...
        self.laplace_comp = D.Laplace(
            self.laplace_comp.loc.detach().cpu(), self.laplace_comp.scale.detach().cpu()
        )
